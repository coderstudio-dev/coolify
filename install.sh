...
if [ $DEBUG -eq 1 ]; then
    ...
    echo "COOLIFY_DATABASE_URL=${COOLIFY_DATABASE_URL:-../db/dev.db}"
...
sudo docker pull -q rcdelacruz/coolify:$VERSION > /dev/null
echo "Starting Coolify."
cd ~/coolify && sudo docker run -tid --env-file $COOLIFY_CONF_FOUND -v /var/run/docker.sock:/var/run/docker.sock -v coolify-db-sqlite rcdelacruz/coolify:$VERSION /bin/sh -c "env | grep COOLIFY > .env && docker compose up -d --force-recreate" > /dev/null
